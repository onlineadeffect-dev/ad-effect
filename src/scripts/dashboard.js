...
export async function refreshDashboardPerformances() {
  ...
  const userId = user ? (user.id || user.email) : null;
  activeBookingsData = await fetchActiveBookings(userId, user ? user.email : null);
  ...
  container.innerHTML = activeBookingsData.map(b => {
    const bb = b.billboards || b;
    const imgUrl = bb.image_url || 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=800&q=80';
    const billboardId = b.billboard_id || bb.billboard_id || 'U002-A';
    const location = bb.location || 'Bahsas, Tripoli Entrance';
    const impressions = b.impressions_per_week ? `${b.impressions_per_week.toLocaleString()} / week` : '142,500 / week';
  ...
  // Bind click event to load full billboard details
  container.querySelectorAll('.active-perf-card').forEach(card => {
    card.addEventListener('click', () => {
      const bookingId = card.getAttribute('data-booking-id');
      const booking = activeBookingsData.find(item => String(item.id) === String(bookingId)) || activeBookingsData[0];
      if (booking) openActiveBillboardModal(booking);
    });
  });
}
...
function openActiveBillboardModal(booking) {
  ...
  const bb = booking.billboards || booking;
  const billboardId = booking.billboard_id || bb.billboard_id || 'U002-A';

  content.innerHTML = `
    <div style="text-align: left;">
      <h2 style="font-size: 2rem; font-weight: 900; color: var(--primary-red);">${billboardId} - Active Billboard</h2>
      <div style="font-size: 1.1rem; font-weight: 700; color: #333; margin-top: 4px;">${bb.location || 'Tripoli Entrance'}</div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-top: 24px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px; font-weight: 600;">
          ...
          <div><strong>Media Type:</strong> ${bb.type || 'Unipole'}</div>
          <div><strong>Dimensions:</strong> ${bb.size || 'W:147 H:43'}</div>
          <div><strong>Weekly Impressions:</strong> ${booking.impressions_per_week ? booking.impressions_per_week.toLocaleString() : '142,500'}</div>
          <div><strong>Campaign Start:</strong> ${new Date(booking.start_time || '2026-08-01').toLocaleDateString()}</div>
          <div><strong>Campaign End:</strong> ${new Date(booking.end_time || '2026-08-31').toLocaleDateString()}</div>
        </div>
      </div>
    </div>
  `;
...
export async function refreshDashboardRequests() {
  ...
  tbody.innerHTML = requests.map(req => {
    const startDate = new Date(req.start_time).toLocaleDateString();
    const endDate = new Date(req.end_time).toLocaleDateString();


    //const isPending = (req.status || '').toLowerCase().includes('pending');
    //const statusBadge = isPending 
      //? `<span class="status-badge status-pending">Pending Review</span>`
      //: `<span class="status-badge status-approved">In Review</span>`; // status of booking request

            // Get the raw status string (e.g. 'pending', 'approved', 'rejected', 'active')
      const statusText = (req.status || 'pending').toLowerCase();

      // Clean display title (e.g. "pending" -> "Pending")
      const statusLabel = statusText.charAt(0).toUpperCase() + statusText.slice(1);

      // Generate badge with dynamic class name and dynamic label text
      const statusBadge = `<span class="status-badge status-${statusText}">${statusLabel}</span>`;
